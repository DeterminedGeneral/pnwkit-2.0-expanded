 ...
 */
function sphereTiering(alliances) {
    if (alliances[0].nations == undefined)
        throw new Error(`PnwKit: missing alliances.nations`);
    if (alliances[0].nations[0].num_cities == undefined)
        throw new Error(`PnwKit: missing alliances.nations.num_cities`);
    if (alliances[0].nations[0].alliance_position == undefined)
        throw new Error(`PnwKit: missing alliances.nations.alliance_position`);
    if (alliances[0].nations[0].vacation_mode_turns == undefined)
        throw new Error(`PnwKit: missing alliances.nations.vacation_mode_turns`);
...
function combineAlliances(alliances) {
    var _a;
    let nations = [];
    for (let i = 0; i < alliances.length; i++) {
        let filteredAlliances = (_a = alliances[i].nations) === null || _a === void 0 ? void 0 : _a.filter((nation) => nation.alliance_position != "APPLICANT" && nation.vacation_mode_turns == 0);
